...
#define	PISTOL_ACCURACY_BURST_SHOT_PENALTY_TIME	0.1f	// Applied amount of time each burst shot adds to the time we must recover from
...
public:
	...
	void	GenericAttack( float soundDuration );
...
BEGIN_DATADESC( CWeaponPistol )
	...
	DEFINE_FIELD( m_nNumBurstShots,			FIELD_INTEGER ),
	DEFINE_FIELD( m_flNextBurstShot,		FIELD_TIME ),
...
void CWeaponPistol::GenericAttack( float soundDuration )
{
	...
	CSoundEnt::InsertSound( SOUND_COMBAT, GetAbsOrigin(), SOUNDENT_VOLUME_PISTOL, soundDuration, GetOwner() );

	BaseClass::PrimaryAttack();

	m_iPrimaryAttacks++;
	gamestats->Event_WeaponFired( ToBasePlayer( GetOwner() ), true, GetClassname() );
}
...
void CWeaponPistol::SecondaryAttack( void )
{
	...
	m_nNumBurstShots = 3;
	if ( m_iClip1 < m_nNumBurstShots )
	{
		m_nNumBurstShots = m_iClip1;
	}
...
void CWeaponPistol::ItemPostFrame( void )
{
	...
	if ( m_nNumBurstShots > 0 )
	{
		if ( m_flNextBurstShot <= gpGlobals->curtime )
		{
			// do not fire more shots than in the burst
			int iClip = m_iClip1;
			m_iClip1 = m_nNumBurstShots;
			// suppress !HEV_AMO0
			pOwner->GiveAmmo( iClip - m_nNumBurstShots, m_iPrimaryAmmoType, true );

			m_flNextPrimaryAttack = m_flNextBurstShot;
			GenericAttack( 0.1f );

			int iSpent = m_nNumBurstShots - m_iClip1;
			m_iClip1 = iClip - iSpent;
			pOwner->RemoveAmmo( iClip - m_nNumBurstShots, m_iPrimaryAmmoType );
			m_nNumBurstShots -= iSpent;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CWeaponPistol::CanHolster( void )
{
	return m_nNumBurstShots == 0 && BaseClass::CanHolster();
}
